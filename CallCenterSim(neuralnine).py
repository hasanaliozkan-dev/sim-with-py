...
class CallCenter:
    ...
    def support(self,customer):
        ...
        print(f"Support finish for {customer} at {self.env.now:.2f}")


def customer(env, name, call_center):
    global customers_handeled
    print(f"Customer {name} enters waiting queue at {env.now:.2f}!")
    with call_center.staff.request() as request:
        yield request
        print(f"Customer {name} enters support room at {env.now:.2f}!")
...
print("Customers Handeled: ",str(customers_handeled))